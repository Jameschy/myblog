/**
 * @description passport 非真实性用户提示
 * @class
 * @name passport.pop.realUserTag
...
passport.realUserTag = passport.realUserTag || {};
passport.realUserTag.data = passport.realUserTag.data || {};
(function(ns) {
        ...
        ns.request = Request;
})(passport.realUserTag.data)


magic.passport.realUserTag = baidu.lang.createClass(function(options){
    var me = this;
    function guid(){
        if(baidu && baidu.guid){
    ...
    me.usermsg = {};
    me.url = {
        REQUEST:'https://passport.baidu.com/v2/api/checkfacticity',
        ADDCOOKIE:"https://passport.baidu.com/v2/?ucentersetcookie",
        BIND:'http://passport.baidu.com/v2/accountsecurity',
        FEEDBACK:'http://passport.baidu.com/v2/?ucenterfeedback&tpl='+me.config.product+'#login'
    }
    me.constant = {
            CONTAINER_CLASS: 'tang-pass-realusertag',
    ...
    type: "magic.passport.realUserTag",
    ...
    _checkuser:function(callback,type){//检查用户是否要弹窗,如果是返回true
        var me = this,
            jsonp = passport.realUserTag.data.request.jsonp,
            url = me.url.REQUEST;
        if(type == 'close'){
            jsonp(url,{act:'close'},{processData:function(){}})
            callback();
        }else{
            jsonp(url,{},{
                processData:function(data){
                    if(data.errno!='110000'){ return true}
                        
                    if((typeof data).toLowerCase() == 'object' && data.username && !data.facticity_hide_notice && !data.facticity){
                        me.usermsg.facticity = false;
                        me.usermsg.username = data.username;
                        me.usermsg.msg = '<p>小伙伴们都绑定手机/邮箱啦，再不绑定你就OUT了！</p>';
                        callback();
                    }else if((typeof data).toLowerCase() == 'object' && !data.facticity_hide_notice && data.display){
                        me.usermsg.facticity = true;
                        me.usermsg.username = data.username;
                        me.usermsg.msg = '<p>使用手机/邮箱登录，随时随地掌握帐号信息，确保安全</p>'+
                                         '<p>您的手机：<span>'+(data.mobile||'您没有绑定密保邮箱')+'</span></p>'+
                                         '<p>您的密保邮箱：<span>'+(data.email||'您没有绑定手机')+'</span></p>';
                        callback();
                    }else{
                        return true
                    }
    ...
    _getTemplate:function(field){
        var Base = passport.realUserTag.data.base,
            getId = this._ids,
            config = this.config,
            ...
            template = {
                wrapper:''+
                    '<div id='+getId+'__wrapper'+' class="realusertag-wrapper" style="width:'+config.width+'px;height:'+config.height+'px">'+
                    '<div id='+getId+'__content'+' class="realusertag-content">'+
                        '<div id='+getId+'__top'+' class="realusertag-top">'+
                            '<h3 class="top-title">'+config.title+'</h3>'+
                            '<a id='+getId+'__top_close'+' class="top-close"></a>'+
                        '</div>'+
                        '<div id='+getId+'__article'+' class="realusertag-article">'+
                            '<div class="article-msg">'+
                                '<p class="article-msg-title">亲爱的'+usermsg.username+'：</p>'+
                                '<div class="article-msg-content">'+usermsg.msg+'</div>'+
                            '</div>'+
                            '<div id='+getId+'__bindbutton'+' class="article-bindbutton '+(usermsg.facticity?'real':'unreal')+'">'+
                                '<div class="bindbutton-wrapper"><a href="'+url.BIND+'" target="_blank" class="bindbutton-a" id="'+getId+(usermsg.facticity?'__realbtn':'__unrealbtn')+'">'+(usermsg.facticity?'下次启用手机/邮箱登录':'快速绑定')+'</a></div>'+
                            '</div>'+
                            '<div id='+getId+'__result'+' class="article-result">'+
                                '<div id='+getId+'__result_title'+' class="result-title"></div>'+
                                '<div id='+getId+'__result_msg'+' class="result-msg"></div>'+
                            '</div>'+
                        '</div>'+
                        '<div id='+getId+'__footer'+' class="realusertag-footer">'+
                            '<p class="footer-left">感谢您的支持!</p>'+
                            '<a class="footer-right" href="' + url.FEEDBACK + '" target="_blank">帮助中心</a>' +
                        '</div>'+
                    '</div>'+
                    '</div>'
            };
    ...
    _initApi:function(){
        var me = this,
            network = passport.realUserTag.data;

        me._checkuser(function(){
            var cssDomain = 'https://passport.baidu.com' ;
            me._loadCss(cssDomain + '/passApi/css/uni_realusertag.css');
            document.body.appendChild(baidu(me._getTemplate('wrapper')).get(0));
            me.addEvent()
        })
    },
    addEvent:function(){
        var me = this,
            Base = passport.realUserTag.data.base,
            $close = this.getElement('top_close'),
            $btn = this.getElement('realbtn'),
            $wrapper = this.getElement('wrapper');
        Base.on($close,'click',function(){
            me._checkuser(function(){
                $wrapper.style.display = 'none';
            },'close')            
        })
        if($btn){
            Base.on($btn,'click',function(event){
                if ( event && event.preventDefault ){ 
                    event.preventDefault();
                } else { 
                    window.event.returnValue = false;
                }
                var jsonp = passport.realUserTag.data.request.jsonp;
                jsonp(me.url.ADDCOOKIE,{},{
                ...
                $btn.innerHTML = '正在启用，请稍后…';
            })            
        }
        if(Base.browser.ie <7){
            window.onScroll = function(){
                me.getElement('wrapper').style.display = 'none';
                if(document.body.scrollTop){
                    me.getElement('wrapper').style.top = document.body.scrollTop+document.body.clientHeight-me.config.height-4+'px'
                }
                me.getElement('wrapper').style.display = '';
            }
        }
    },
    hide:function(){
        this.getElement('wrapper').style.display = 'none'
    },
    getElement:function(str){
        return document.getElementById(this._ids+'__'+ str)
    }
})
